function PubBarChart(area, data, options) {
    this.cnfg = {
        marginTop: 40,
        marginRight: 20,
        marginBottom: 40,
    ...
    this.globalBarNum = globalPubBarNumber++;
	
    $("body").append('<div id="pub-bar-tooltip' + this.globalBarNum + '" class="bar-tooltip"></div>');
    $("#pub-bar-tooltip" + this.globalBarNum).css({"position": "absolute", "background": "rgba(0, 0, 0, 0.8)", "color": "white", "font-family": "Arial", "font-size": "14px", "font-weight": "lighter", "z-index": "100", "pointer-events": "none"});
    $("#pub-bar-tooltip" + this.globalBarNum).append('<div id="pub-bar-tooltip-content' + this.globalBarNum + '" style="padding: 5px 10px 5px 10px;">');
    $("#pub-bar-tooltip-content" + this.globalBarNum).append('<span id="pub-bar-tooltip-desc' + this.globalBarNum + '" style="font-size: 12px;">desc of tooltip</span><br/>');
    $("#pub-bar-tooltip-content" + this.globalBarNum).append('<span id="pub-bar-tooltip-value' + this.globalBarNum + '" style="font-size: 16px;">value of tooltip</span>');

    $("#pub-bar-tooltip" + this.globalBarNum).append('<svg style="position: absolute; width: 12px; height: 12px;"><polygon points="0,0 12,0 6,10" style="background: black; fill: rgba(0, 0, 0, 0.8);"></polygon></svg>');

    $("#pub-bar-tooltip" + this.globalBarNum).hide();
    ...
    this.draw = function() {
		...
		$("#pub-bar-tooltip" + parseInt(barNum)).hide();
        ...
        barChartGroup.selectAll("rect")
                .on("mouseover", function(d, i) {
                    ...
                    d3.select("#pub-bar-tooltip-desc" + barNum).text(d.name);
                    var tipValue = parseFloat(d.value).toFixed(cnfg.valuePrecision);
                    tipValue = +tipValue;
                    d3.select("#pub-bar-tooltip-value" + barNum).text(tipValue);
                    d3.select("#pub-bar-tooltip" + barNum)
                            .transition()
                            .style("display", "block")
                            .style("left", posX + "px")
                            .style("top", posY + "px");
				...
				.on("click", function(d, i){
					if (d.type == "aoi") {
						fetchPubsCountData("saoi", d.id, d.name);
					} else if (d.type == "saoi"){
						fetchPubsCountData("ssaoi", d.id, d.name);
					} 
				});
