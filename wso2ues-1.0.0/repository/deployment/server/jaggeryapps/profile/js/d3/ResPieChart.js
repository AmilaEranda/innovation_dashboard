function ResPieChart(area, data, options) {
    var cnfg = {
        marginTop: 20,
        marginRight: 20,
        marginBottom: 20,
    ...
    var legendWidth = 120;
	var initialDivHeight = $(areaId).height();

    this.globalPieNum = globalResPieNumber++;

    $("body").append('<div id="res-pie-tooltip' + this.globalPieNum + '" class="pie-tooltip"></div>');
    $("#res-pie-tooltip" + this.globalPieNum).css({"position": "absolute", "background": "rgba(0, 0, 0, 0.8)", "color": "white", "font-family": "Arial", "font-size": "14px", "font-weight": "lighter", "z-index": "100", "pointer-events": "none"});
    $("#res-pie-tooltip" + this.globalPieNum).append('<div id="res-pie-tooltip-content' + this.globalPieNum + '" style="padding: 5px 10px 5px 10px;">');
    $("#res-pie-tooltip-content" + this.globalPieNum).append('<span id="res-pie-tooltip-desc' + this.globalPieNum + '" style="font-size: 12px;">desc of tooltip</span><br/>');
    $("#res-pie-tooltip-content" + this.globalPieNum).append('<span id="res-pie-tooltip-value' + this.globalPieNum + '" style="font-size: 16px;">value of tooltip</span>');

    $("#res-pie-tooltip" + this.globalPieNum).hide();
    ...
    this.draw = function() {
		...
		$("#res-pie-tooltip" + parseInt(pieNum)).hide();
        ...
        var chartHeight = initialDivHeight - chartMargin.top - chartMargin.bottom;
		...
		$(areaId).height(initialDivHeight + additionalHeight);

        var svg = d3.select(areaId)
                .append("svg")
                .attr("width", chartWidth + chartMargin.left + legendWidth + chartMargin.right)
                .attr("height", chartHeight + chartMargin.top + chartMargin.bottom + additionalHeight)
        ...
        arcGroup.selectAll("path")
                .on("mouseover", function(d) {
                    ...
                    d3.select("#res-pie-tooltip-desc" + pieNum).text(d.data.name);
                    d3.select("#res-pie-tooltip-value" + pieNum).text(d.data.value);
                    d3.select("#res-pie-tooltip" + pieNum)
                            .transition()
                            .style("display", "block")
                ...
                .on("click", function(d, i) {
					if (d.data.type == "aoi") {
						fetchPeopleCountData("saoi", d.data.id, d.data.name);
					} else if (d.data.type == "saoi"){
						fetchPeopleCountData("ssaoi", d.data.id, d.data.name);
					} 
                });
