...
function fetchPiNIData() {
	var url =  "../../portal/gadgets/gii-2014-map/data-files/gii-2014-pillars-ni-data.jag";
	$.ajax({
		url: url,
		type: "GET",
		dataType: "json",
...
function fetchCustomData() {
	...
	var url = "../../portal/gadgets/gii-2014-map/data-files/gii-2014-map-data.jag";
...
function drawMap(){
	...
	map.titles = [];
	if (pillarType === "gv") {
		map.addTitle("The Global Innovation Index 2014", 18, "#000000", 1, true);// (text, size, color, alpha, bold)
	} else if (pillarType === "pi" && niId === "0") {
		var pillarName = $("#pillar-selection option:selected").text();
		map.addTitle("GII 2014 - " + pillarName, 16, "#000000", 1, true);
	} else {
		var pillarName = $("#pillar-selection option:selected").text();
...
function createList(){
	...
	if(pillarType === "gv"){
		titleText = document.createTextNode("The Global Innovation Index 2014");
	} else if (pillarType === "pi" && niId === "0") {
		var pillarName = $("#pillar-selection option:selected").text();
		titleText = document.createTextNode("GII 2014 - " + pillarName);
	} else {
		var pillarName = $("#pillar-selection option:selected").text();
		var niName = $("#ni-selection option:selected").text();
	...
	text = document.createTextNode("Source: Cornell University, INSEAD, and WIPO (2014): The Global Innovation Index 2014, ");
...
function initMap(){
	...
	map.pathToImages = "../../portal/gadgets/gii-2014-map/css/ammap/images/";
	...
	map.areasSettings = {
		...
		color: "#A9D6F4", // Color of area with lowest value
		colorSolid: "#0063A7", // Color of area with highest value
		outlineColor: "#dddddd",
	...
	map.zoomControl = {
		buttonFillColor: "#249FF3",
		buttonRollOverColor: "#0063A7"
	};
...
function drawIndexBarChart(data){
	...
	if (pillarType === "gv") {
		indexBarChartOptions = {
			chartTitle: "The Global Innovation Index 2014"
		};
	} else if (pillarType === "pi" && niId === "0") {
		var pillarName = $("#pillar-selection option:selected").text();
		indexBarChartOptions = {
			chartTitle: "GII 2014 - " + pillarName
		};
	} else {
		var pillarName = $("#pillar-selection option:selected").text();
		var niName = $("#ni-selection option:selected").text();
...
function setInfoForCharts(){
	...
	if(pillarType === "gv"){ // global values
		$("#info-icon").tooltip({
			...
			placement: "left",
			html: true,
			title: '<div style="text-align: left; font-size: 11px;"><h5><i class="fa fa-info-circle fa-lg"></i> GII - 2014</h5><p>A composite indicator that ranks countries/economies in terms of their enabling environment to innovation and their innovation outputs. It is co-published by Cornell University, INSEAD and the World Intellectual Property Organization (WIPO).</p></div>'
		});
