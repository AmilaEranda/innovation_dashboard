...
function fetchPiNIData() {
	var url =  "../../portal/gadgets/gii-2011-map/data-files/gii-2011-pillars-ni-data.jag";
	$.ajax({
		url: url,
		type: "GET",
		dataType: "json",
...
function fetchCustomData() {
	...
	var url = "../../portal/gadgets/gii-2011-map/data-files/gii-2011-map-data.jag";
...
function drawMap(){
	...
	map.titles = [];
	if (pillarType === "gv") {
		map.addTitle("The Global Innovation Index 2011", 18, "#000000", 1, true);// (text, size, color, alpha, bold)
	} else if (pillarType === "pi" && niId === "0") {
		var pillarName = $("#pillar-selection option:selected").text();
		map.addTitle("GII 2011 - " + pillarName, 16, "#000000", 1, true);
	} else {
		var pillarName = $("#pillar-selection option:selected").text();
...
function createList(){
	...
	if (pillarType === "gv") {
		titleText = document.createTextNode("The Global Innovation Index 2011");
	} else if (pillarType === "pi" && niId === "0") {
		var pillarName = $("#pillar-selection option:selected").text();
		titleText = document.createTextNode("GII 2011 - " + pillarName);
	} else {
		var pillarName = $("#pillar-selection option:selected").text();
		var niName = $("#ni-selection option:selected").text();
	...
	text = document.createTextNode("Source: INSEAD, and WIPO (2011): The Global Innovation Index 2011, ");
...
function initMap(){
	...
	map.pathToImages = "../../portal/gadgets/gii-2011-map/css/ammap/images/";
	...
	map.areasSettings = {
		...
		color: "#ddd0ec", // Color of area with lowest value
		colorSolid: "#552989", // Color of area with highest value
		outlineColor: "#dddddd",
	...
	map.zoomControl = {
		buttonFillColor: "#9549ee",
		buttonRollOverColor: "#552989"
	};
...
function drawIndexBarChart(data){
	...
	if (pillarType === "gv") {
		indexBarChartOptions = {
			chartTitle: "The Global Innovation Index 2011"
		};
	} else if (pillarType === "pi" && niId === "0") {
		var pillarName = $("#pillar-selection option:selected").text();
		indexBarChartOptions = {
			chartTitle: "GII 2011 - " + pillarName
		};
	} else {
		var pillarName = $("#pillar-selection option:selected").text();
		var niName = $("#ni-selection option:selected").text();
...
function setInfoForCharts(){
	...
	if(pillarType === "gv"){ // global values
		$("#info-icon").tooltip({
			...
			placement: "left",
			html: true,
			title: '<div style="text-align: left; font-size: 11px;"><h5><i class="fa fa-info-circle fa-lg"></i> GII - 2011</h5><p>A composite indicator that ranks countries/economies in terms of their enabling environment to innovation and their innovation outputs. It is co-published by Cornell University, INSEAD and the World Intellectual Property Organization (WIPO).</p></div>'
		});
